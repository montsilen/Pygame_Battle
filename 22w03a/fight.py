...
# Create entity
class entity():
    def __init__(self, posx, posy, width, height, collide, gravity, side, speed, path, maxblood, dmg=0, life=True, aoe=False):
...
ABLE1 = "able1"
class player(entity):
    def __init__(self, posx, posy, width, height, side, speed, path, blood, faceright=True):
...
# Blood stripes
numberimg = pygame.image.load("./numbers.png").convert_alpha()
def drawnumbers(numbers):
    surface = pygame.Surface((10*len(numbers)-2, 16)).convert_alpha()
    surface.fill((0, 0, 0, 0))
    for i in range(len(numbers)):
        if numbers[i] == "/":
            surface.blit(numberimg, (i*10, 0), (80, 0, 8, 16))
        else:
            surface.blit(numberimg, (i*10, 0), (int(numbers[i])*8, 0, 8, 16))
    return surface
...
def drawHp(surface, p1, p2, w, s, p):
    ...
    if b1 > 0:
        ...
        w1 = drawnumbers("{}/{}".format(p1.blood, p1.maxblood))
        r1 = w1.get_rect()
        r1.center = (p, 16)
        surface.blit(w1, r1)
    if b2 > 0:
        pygame.draw.rect(surface, (255, 0, 0), (SCREEN[0]-s-15-b2, 4, b2, 24), 0)
        w2 = drawnumbers("{}/{}".format(p2.blood, p2.maxblood))
        r2 = w2.get_rect()
...
player2 = player(SCREEN[0], 0, 32, 32, 2, 4, "man2.png", 4620)
